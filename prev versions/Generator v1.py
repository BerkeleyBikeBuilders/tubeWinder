...
# This is the function that write code for non-linear tubes
    ...
    while current < passes:
        ...
        file_saver(non_lin_sec(p0, p1, length_2, wrap_angle, direction, feedrate), file_name)
...
# This is the function that write code for non-linear tubes
    ...
    while current < passes:
        ...
        print(non_lin_sec(p0, p1, length_2, wrap_angle, direction, feedrate))
...
def lin_sec(p, length, wrap_angle, direction, feedrate):
    Dest_X = length
    Dest_Y = ((2 * np.pi*(np.tan(np.radians(wrap_angle))) * length) / p)
    if direction == 1:
...
# Example Command: G5 E(Line Legnth) F(Feedrate) I(Ctrl1-x) J(Ctrl1-y) P(Ctrl2-x) Q(Ctrl2-y) X(Dest-x) Y(Dest-y)
def non_lin_sec(p0, p1, length, wrap_angle, direction, feedrate):
    Ctrl1_x = (length/3) // 1
    Ctrl1_y = (2 * np.pi * np.tan(np.radians(wrap_angle)) * Ctrl1_x) / (p0 + ((Ctrl1_x/length)*(p1-p0))) // 1
    Ctrl2_x = (length * 2/3) // 1
    Ctrl2_y = (2 * np.pi * np.tan(np.radians(wrap_angle)) * Ctrl2_x) / (p0 + ((Ctrl2_x/length)*(p1-p0))) // 1
    Dest_x = length
    Dest_y = (2 * np.pi * np.tan(np.radians(wrap_angle)) * Dest_x) / (p0 + ((Dest_x/length)*(p1-p0))) // 1
    Line_Length = line_length(p0, p1, length, wrap_angle) // 1
    if direction == 1:
        return f'G5 E{Line_Length} F{feedrate} I{Ctrl1_x} J{Ctrl1_y} P{Ctrl2_x} Q{Ctrl2_y} X{Dest_x} Y{Dest_y}'
    if direction == -1:
        f'G5 E{Line_Length} F{feedrate} I{-Dest_x + Ctrl2_x} J{-Dest_y + Ctrl2_y} P{-Dest_x + Ctrl1_x} Q{-Dest_y + Ctrl1_y} X{-Dest_x} Y{-Dest_y}'
...
def passes_calculator(tube_thickness):
    return (tube_thickness * (1 / 0.18)) // 1
