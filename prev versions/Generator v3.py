...
# This is the  fuction that writes code for linear tubes
    file_saver((f'G01 G91 F{feedrate}'), file_name)
...
# This is the function that write code for non-linear tubes
    file_saver((f'G01 G91 F{feedrate}'), file_name)
    file_saver((f'F{feedrate}'), file_name)
...
def passes_calculator(tube_thickness, length, perimeter, tow_width, wrap_angle):
    layers = round((tube_thickness * (32 / 2.4)), 0)
    passes = layers * ((length * perimeter) / (tow_width * np.sqrt(length ** 2 + (length * np.tan(wrap_angle)))))
    return passes
...
#non_linear_terminal(104.922, 144, 16, 465.162, 16, 60, 0.2, 6.5, 12500)
#file_saver("hello", 'a.txt')